...
export default class FrontierEthProvider extends Provider {
  private eth = (api.rpc as any).evm;
  ...
  async getTransactionCount(
    ...
    if (blockTag) logger.warn(`Provided parameter 'blockTag' will not be used`);
    return this.eth.getTransactionCount(await addressOrName).then((r: any) => r.toNumber());
  }

  async getCode(addressOrName: string | Promise<string>, blockTag?: BlockTag | Promise<BlockTag>): Promise<string> {
    if (blockTag) logger.warn(`Provided parameter 'blockTag' will not be used`);
    return this.eth.getCode(await addressOrName).then((r: Codec) => r.toHex());
  }
  ...
  async getStorageAt(
    ...
    if (blockTag) logger.warn(`Provided parameter 'blockTag' will not be used`);
    return this.eth
      .getStorageAt(await addressOrName, BigNumber.from(await position).toBigInt())
      .then((r: Codec) => r.toHex());
  }
  ...
  async call(transaction: Deferrable<TransactionRequest>, blockTag?: BlockTag | Promise<BlockTag>): Promise<string> {
    ...
    // @ts-ignore
    const r: Bytes = await this.eth.call({
      ...tx,
