      ...
      it('can filter for contract creation', async () => {
        ...
        expect(
          processor.filterProcessor({
            filter: {},
            input: contractTx,
            ds: {processor: {options: {address: null}}} as unknown as FrontierEvmDatasource,
          })
        ).toBeTruthy();
      }, 40000);
  ...
  describe('FrontierTransformation', () => {
    ...
    describe('FrontierEvmCalls', () => {
      ...
      it('can transform a contract creation tx', async () => {
        ...
        expect(call.to).not.toBeDefined(); // Newly created contract address should be fetched through the receipt
      ...
      it('can transform a legacy contract creation tx', async () => {
        // https://moonriver.subscan.io/block/4309725
        // https://blockscout.moonriver.moonbeam.network/blocks/4309725/transactions

        const blockNumber = 4309725;
        const {extrinsics} = await fetchBlock(api, blockNumber);

        const [call] = (await processor.transformer({
          input: extrinsics[7] as SubstrateExtrinsic<[TransactionV2 | EthTransaction]>,
          ds: baseDS,
          api,
        })) as [FrontierEvmCall];

        expect(call.to).not.toBeDefined();
      }, 10000);
      ...
      it.skip('can transform a failed tx without execution event', async () => {
        // https://moonbase.subscan.io/block/131451?tab=extrinsic
        // https://moonbase-blockscout.testnet.moonbeam.network/block/131451/transactions
